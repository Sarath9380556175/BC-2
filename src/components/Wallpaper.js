...
class Wallpaper extends React.Component{
  ...
  componentDidMount()
  {
    ...
    }).then(res=>this.setState({getsignupdetails:res.data.signupdetails, mails:res.data.signupdetails.map((item)=>{return item.email}), passwords:res.data.signupdetails.map((item)=>{return item.password})}))
...
this.setState({[name]:value})
       ...
       handlegender=(gender)=>{
         const {email,password,mails,getsignupdetails,passwords}=this.state;
         const gmail=mails.indexOf(email)
         const pwd=passwords.indexOf(password)
         console.log(mails)
    ...
    data:
    {
      email:mails[gmail]==email && passwords[pwd]==password ? email:null,
      password:passwords[pwd]==password?password:alert('invalid password'),
      gender:gender
    }
         })
   ...
   this.setState({[name]:value})
     ...
     console.log(mailing)
       ...
       this.props.history.push(`/otp/?email=${emails}&mobilenumber=${mobilenumber}`)
       ...
       responseGoogle=(response)=>{
         ...
         const ukp=localStorage.setItem('skr',response.profileObj.name)
         this.setState({googleusername:localStorage.getItem('skr'),IsGoogleuserLoggedIn:true,islogin:false,googleimage:response.profileObj.imageUrl})
       }
...
this.setState({[state]:value})
       ...
       passwordmatch=()=>{
         this.setState({ispasswordverified:false,iscreatingaccount:true})
       }
      
    render()
    {
        const {locations}=this.props;
        const {ispasswordverified,islogin,IsuserLoggedIn,logindetails,iscreatingaccount,isuserwanttologin,IsGoogleuserLoggedIn,googleusername,facebookusername,isfacebookuserloggedIn,image,googleimage,restaurants}=this.state;
        return(
        <div>
  ...
  &nbsp;
    <ul class="navbar-nav"> 
      <li class="nav-item">
        <motion.button class="nav-link btn  text-danger btn-sm" onClick={this.Aboutus}>About us</motion.button>
      </li>
      &nbsp;
      <li class="nav-item">
...
<div className="container-fluid jumbotron text-center" style={{background:"url(Assets/restaurantlogo.jpg)"}}>
  ...
  <div className="logo">
  <motion.p className="skrs">{googleimage ? <img src={`${googleimage}`} alt="Nothing Found" className="gimage rounded-circle"/> :<motion.div initial={{y:400}}  animate={{y:-5}} >SKR  <i className="fas fa-coffee bg-warning rounded" id="top"></i> <b>R</b>estaurant</motion.div>}</motion.p>
</div>
<br/><br/>
...
<Modal
isOpen={ispasswordverified}

style={modalstyles}
>
 <div style={{float:'right'}} className="mr-1" onClick={this.passwordmatch}>close</div> 
 <br/>
 <br/>
 <div id="message">
...
<div className="text-center">
    ...
    <div onClick={()=>this.handleaccount('iscreatingaccount',false)} style={{float:'right',marginTop:'-49px',fontSize:'20px',fontStyle:'italic',color:'gold',marginRight:'-16px'}}>close</div>
       ...
       <i className="fas fa-lock"></i>&nbsp;&nbsp;<input type="password" name="passwords" id="pass" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"    required  onChange={this.handlepasswords} /><br/><br/>
